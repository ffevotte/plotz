...
depending on it is drawn."""
    ...
    def draw (self, position):
        ...
        # Axis
        out.write ("\\draw (%s)" % self._coord(self.min, position))
        out.write (" -- (%s);\n" % self._coord(self.max, position))
        ...
        if self.label is not None:
            ...
            out.write (" [%s] {%s};\n" % (label_position, self.label))

        # Ticks
        for (x,label) in self.ticks:
            out.write ("\\draw (%s)" % self._coord("%.15f"%x,
                                                   "%.15f+%.15f/%s"
                                                   %(position, tick_size, scale)))
            out.write (" -- (%s)" % self._coord("%.15f"%x,
                                                 "%.15f-%.15f/%s"
                                                 %(position, tick_size, scale)))
            out.write (" node [%s] {%s};\n"   % (tick_position, label))
        ...
        if self.subtick > 0:
            ...
            while x <= self.max:
                label = self.tick_format (x)
                out.write ("\\draw[style=help lines]")
                out.write (" (%s)" % self._coord(x, position+tick_size/2))
                out.write (" -- (%s);\n" % self._coord(x, position-tick_size/2))
                x += self.subtick
...
class Plot:
    ...
    def colormap (self, name=None):
        ...
        self.header += "%\n".join([r"\definecolor{color%d}{HTML}{%s}" % (i+1, c[i]) for i in xrange(len(c))])
    ...
    def style (self, line, style):
        self.header += r"\tikzset{%s/.append style={%s}}" % (line, style) + "%\n"
    ...
    def __enter__ (self):
        ...
        with open(os.path.join(self._tmpdir.name(), "standalone.tex"), "w") as f:
            f.write ("\n".join([
        ...
        self._out = open (os.path.join(self._tmpdir.name(), "pykzplot.tex"), "w")
        self._out.write (self.header + "%\n")
        self._out.write ("%\n".join([
            r"\pgfmathparse{%f*\tikzscale}" % self.xscale,
            r"\edef\XXX{\pgfmathresult}",
            r"\pgfmathparse{%f*\tikzscale}" % self.yscale,
            r"\edef\YYY{\pgfmathresult}",
            r"\begin{tikzpicture}[x=%f,y=%f,scale=\tikzscale]" % (self.xscale, self.yscale),
        ]))
...
The graph is then rendered."""
        ...
        self._out.write ("\\end{tikzpicture}\n")
        self._out.close()

        subprocess.call(["pdflatex", "standalone"],
                        cwd=self._tmpdir.name())
        subprocess.call(["cp",
                         os.path.join(self._tmpdir.name(), "standalone.pdf"),
                         self._output+".pdf"])
        subprocess.call(["cp",
                         os.path.join(self._tmpdir.name(), "pykzplot.tex"),
                         self._output+".tex"])
        #subprocess.call ("touch '%s'" % self._output, shell=True)

    def xgrid (self):
        """Output the necessary TikZ code to draw a grid"""
        for (x,_) in self.x.ticks:
            self._out.write ("\\draw[style=help lines]" +
                             "(%.15f,%.15f) -- (%.15f,%.15f);\n"
                             % (x, self.y.min, x, self.y.max))
...
(x0, y0): position of the top-left corner of the legend"""
        ...
        for l in self._lines:
            if l.line:
                self._out.write ("\\draw[line%s]" % self._line_index(l))
                self._out.write (" (%f,%f-%d/\\YYY)"    % (x0, y0, k))
                self._out.write (" -- (%f+20/\\XXX,%f-%d/\\YYY)" % (x0, y0, k))
                self._out.write (" node [right, black, fill=white, inner sep=2pt] {%s};\n" % l.title)
            if l.points:
                self._out.write ("\\node[line%s]"   % self._line_index(l))
                self._out.write ("at (%f+10/\\XXX,%f-%d/\\YYY)" % (x0, y0, k))
                self._out.write (" {\\marker%s};\n" % self._line_index(l))
            k += 15

    def plot (self, fun=None, data=None, datatransform=None, title=None, samples=1000, line=True, points=False):
        self.line += 1
        if title:
            self._lines.append(Line(self.line, title, line, points))

        self._out.write ("\\draw [line%s] \n" % self._line_index())
        ...
        def plot_point (first, x, y):
            ...
            self._out.write ("(%.15f,%.15f)" % (x, y))

            if points:
                self._out.write (" node {\\marker%s}" % self._line_index())
            self._out.write ("\n")
...
def test1 ():
    ...
    plot.output = "essai1.pdf"

    plot.begin()
    for k in xrange(1, 9):
        plot.plot (fun = lambda x: k*math.cos(k*x)/8)
    plot.end()
...
def test2 ():
    ...
    plot.output = "essai2.pdf"

    plot.begin()
    for k in xrange(8):
        plot.plot (fun = lambda x: x**(k-3.5), samples = 10, line=(k<4), points=True, title="$k = %d$"%k)
    plot.grid()
    plot.legend (-1, 6.5)
    plot.end()
...
def test3 ():
    ...
    with plot:
        plot.grid()
        plot.plot (data = "essai.dat", title=r"$\sin(\pi\,x)$")
